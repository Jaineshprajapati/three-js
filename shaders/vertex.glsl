...
// 1. You forgot to declare 'uniform float uTime;' at the top, so uTime is undefined! 
// 2. You're missing a semicolon at the end of this line: modelPosition.z+=cnoise(vec3(modelPosition.xy,uTime*.01)) 
//    It should be: modelPosition.z += cnoise(vec3(modelPosition.xy, uTime * .01));

uniform float uTime;
varying vec2 vUv;
// Classic 3D Perlin Noise ("cnoise"), by Stefan Gustavson.  
// Modified version for GLSL.
...
void main() {
    ...
    // Compute model, view, and projected positions
    vec4 modelPosition = vec4(position, 1.0);
    noise = cnoise(vec3(modelPosition.xy, uTime * 0.2)) * 0.3;
